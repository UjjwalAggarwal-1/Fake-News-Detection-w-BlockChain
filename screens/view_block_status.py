...
def block_valid():
    return (
        int(time.time()) - st.session_state.p2pserver.received_block.timestamp
        <= config.BLOCK_VALIDATOR_CHOOSE_INTERVAL
    )
...
def propose_block():
    ...
    if st.session_state.screen == "propose_block":
        ...
        if (
            selected_option
            and change_screen_.screen_mapping[selected_option]
            != st.session_state.screen
            and change_screen_.screen_mapping[selected_option] != "view_block_status"
        ):
        ...
        st.markdown(change_screen_.view_block_status_message)

        # IF ALREADY MADE BLOCK
        if st.session_state.p2pserver.received_block:
            st.warning("You have already transmitted the block")
            st.write(
                "Current Confirmations on Block: ",
                len(st.session_state.p2pserver.received_block.votes),
            )
            ...
            if not transactions:
                ...
                for transaction in transactions:
                    ...
                    st.markdown(
                        f"""<span><b>
                            ...
                            Content URL: https://{transaction.ipfs_address}.ipfs.dweb.link</b></span>""",
                        unsafe_allow_html=True,
                    )

                    # WHETHER TO INCLUDE IN BLOCK OR NOT
                    include_value = st.radio(
                        "Include Transaction in Block?",
                        ["False", "True"],
                        key=f"include_{transaction.id}",
                    )

                    # WHETHER NEWS IS FAKE OR TRUE
                    vote_value = st.radio(
                        "Vote on News:", ["Fake", "True"], key=f"vote1_{transaction.id}"
                    )
                ...
                if (
                    st.button("Create Block")
                    and 1 <= len(selected_transactions) <= max_selections
                ):
                    for transaction in selected_transactions:
                        if votes[transaction.id] == "True":
                            transaction.positive_votes.add(public_key)
                        else:
                            transaction.negative_votes.add(public_key)
...
def view_block_status():
    ...
    if (
        selected_option
        and change_screen_.screen_mapping[selected_option] != st.session_state.screen
    ):
        change_screen_.change_screen_navbar(selected_option)
    ...
    if st.session_state.screen == "view_block_status":
        ...
        if (
            st.session_state.p2pserver.block_proposer
        ...
        if (
            st.session_state.p2pserver.received_block
            and block_valid()
            and st.session_state.validator
        ):
        ...
        st.write(
            "Current Block Proposer Public Key: ",
            st.session_state.p2pserver.block_proposer,
        )
        if st.session_state.p2pserver.received_block:
            st.write(
                "Current Confirmations on Receieved Block: ",
                len(st.session_state.p2pserver.received_block.votes),
            )
        else:
