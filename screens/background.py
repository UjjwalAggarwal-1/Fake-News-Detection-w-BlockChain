...
from extra.config import BLOCK_VALIDATOR_CHOOSE_INTERVAL, PENALTY_NO_BLOCK, START_TIME
...
class Background:
    ...
    def can_add_block(self, block):
        ...
        return len(block.votes) >= (
            0.5 * self.p2pserver.accounts.get_count_of_validators()
        )
    ...
    def run_forever(self):
        while True:
            ...
            if time_elapsed % BLOCK_VALIDATOR_CHOOSE_INTERVAL == 0:
                ...
                if self.p2pserver.received_block:
                    ...
                    if self.can_add_block(self.p2pserver.received_block):
                        ...
                        self.p2pserver.accounts.reduce_balance(
                            ...
                            PENALTY_NO_BLOCK,
                            "RECEIVED BLOCK DID NOT GET ENOUGH VOTES",
                        )
                        print("As block proposer, your block didn't get enough votes\n")
                else:
                    if self.p2pserver.transaction_pool.check_oldest_transaction(
                        current_time - BLOCK_VALIDATOR_CHOOSE_INTERVAL
                    ):
                        self.p2pserver.accounts.reduce_balance(
                            self.p2pserver.block_proposer,
                            PENALTY_NO_BLOCK,
                            "BLOCK PROPOSER DIDNT PROPOSE THE BLOCK IN TIME",
                        )
                ...
                self.p2pserver.block_proposer = (
                    self.p2pserver.accounts.choose_validator(current_time // 10)
                )
