...
def show_blocks_news():
    ...
    if st.session_state.screen == "show_blocks":
        ...
        st.markdown(change_screen_.view_block_news_message, unsafe_allow_html=True)
            ...
            for block in chain:
                for transaction in block.transactions:
                    percent_fake_votes = 100 * (
                        ...
                        / (
                            len(transaction.negative_votes)
                            + len(transaction.positive_votes)
                        )
                    )
                    ...
                    table_data.append(
                        {
                            ...
                            "Total Votes": len(transaction.positive_votes)
                            + len(transaction.negative_votes),
                            ...
                            + ".ipfs.dweb.link",
                            "ID": transaction.id,
                            "Transaction Creation Time": datetime.fromtimestamp(
                                transaction.timestamp
                            ).strftime("%I:%M %p on %d %B, %Y"),
                            "Block Creation Time": datetime.fromtimestamp(
                                block.timestamp
                            ).strftime("%I:%M %p on %d %B, %Y"),
                            ...
                            "qr code": st.button(
                                f"QR for {transaction.id}",
                                on_click=show_transaction,
                                kwargs={"transaction": transaction, "show": 1},
                            ),
                        }
                    )
