...
def view_sent_news():
    if st.session_state.screen == "view_sent_news":
        ...
        st.markdown(change_screen_.view_sent_news_message, unsafe_allow_html=True)
        ...
        if len(sent_transactions) < 1:
            st.write("You haven't broadcasted any news in the network yet.")
            ...
            for transaction in sent_transactions:
                status_pool = (
                    st.session_state.p2pserver.transaction_pool.transaction_exist(
                        transaction
                    )
                )
                        ...
                        for verified_transaction in block.transactions:
                            if transaction.id == verified_transaction.id:
                                percent_fake_votes = 100 * (
                                    len(verified_transaction.negative_votes)
                                    / (
                                        len(verified_transaction.negative_votes)
                                        + len(verified_transaction.positive_votes)
                                    )
                                )
                                transaction_data["Percent of Fake Votes"] = (
                                    str(percent_fake_votes) + "%"
                                )
                                transaction_data["Percent of True Votes"] = (
                                    str(100 - percent_fake_votes) + "%"
                                )
                                transaction_data["Total Votes"] = (
                                    len(verified_transaction.positive_votes)
                                    + len(verified_transaction.negative_votes),
                                )
